...
Function InputBox$(x%, y%, width%, height%, Txt$, ID% = 0)
	...
	If MouseOn(x, y, width, height) Then
		Color(50, 50, 50)
		MouseOnBox = True
		If Input_MouseDown() Then SelectedInputBox = ID : FlushKeys
	EndIf
	
	Rect(x + 2, y + 2, width - 4, height - 4)
	Color (255, 255, 255)	
	
	If (Not MouseOnBox) And Input_MouseDown() And SelectedInputBox = ID Then SelectedInputBox = 0
...
Function DrawButton%(x%, y%, width%, height%, txt$, bigfont% = True, waitForMouseUp%=False, usingAA%=True)
	...
	If MouseOn(x, y, width, height) Then
		Color(30, 30, 30)
		If (Input_MouseDown() And (Not waitForMouseUp)) Or (MouseUp1 And waitForMouseUp) Then 
			clicked = True
...
Function DrawTick%(x%, y%, selected%, locked% = False)
	...
	If Highlight Then
		Color(50, 50, 50)
		If Input_MouseDown() Then selected = (Not selected) : PlaySound_Strict (UI_ButtonSFX)
	Else
		Color(0, 0, 0)		
	End If
